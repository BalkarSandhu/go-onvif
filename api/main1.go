...
func getPTZStructByName(name string, dev *onvif.Device, acceptedData []byte) (interface{}, error) {
...
func getMediaStructByName(name string, dev *onvif.Device, acceptedData []byte) (interface{}, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
...
func callNecessaryMethod(serviceName, methodName string, acceptedData []byte, dev *onvif.Device) (string, error) {
	...
	case "media":
		response, err = getMediaStructByName(methodName, dev, acceptedData)
	default:
